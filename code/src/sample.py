...
def parse_args():
    ...
    parser.add_argument('--random_sample', action='store_true', default=True,
                        help='whether to sample the dataset with a random sampler')
    args = parser.parse_args()
    return args


def build_word_dict(pickle_path):
    pickle_path = os.path.abspath(os.path.join(ROOT_PATH, pickle_path))
    print("Attempting to open:", pickle_path)
...
def main(args):
    ...
    # prepare data
    pickle_path = os.path.join(ROOT_PATH, '..', 'data', 'birds', 'captions_DAMSM.pickle')
    ...
    # prepare models
    _, text_encoder, netG = prepare_models(args)
